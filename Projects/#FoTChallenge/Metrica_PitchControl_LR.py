...
def initialise_players(team,params):
    ...
    """    
    # get player  ids
    player_ids = np.unique( [ c.split('_')[0] for c in team.keys() if c[:4] != 'ball' and c[:4] != 'Time' ] )
    # create list
    team_players = []
    for p in player_ids:
        # create a player object for player_id 'p'
        team_player = player(p,team,params)
...
def default_model_params(time_to_control_veto=3):
    ...
    params['max_player_speed'] = 6. # maximum player speed m/s
