...
def plot_events( events, risky, figax=None, field_dimen = (106.0,68), indicators = ['Marker','Arrow'], color='r', marker_style = 'o', alpha = 0.5, annotate=False):
    """ plot_events( events )
    ...
    for i,row in events.iterrows():
        timestring = '%s passes overplaying 3 or more defenders' % str(row['From'])
        ax.text(-2.5,field_dimen[1]/2.+1., timestring, fontsize=14 )
    ...
    for i,row in risky.iterrows():
        if 'Marker' in indicators:
            ax.plot(  row['Start X'], row['Start Y'], 'k'+marker_style, alpha=alpha )
        if 'Arrow' in indicators:
            ax.annotate("", xy=row[['End X','End Y']], xytext=row[['Start X','Start Y']], alpha=alpha, arrowprops=dict(alpha=alpha,arrowstyle="->",color='k'),annotation_clip=False)
        if annotate:
            textstring = row['Type'] + ': ' + row['From']
            ax.text( row['Start X'], row['Start Y'], textstring, fontsize=10, color='k')
    title = '%s' % str(row['From'])
    fig.savefig(title + '.png', dpi=None, bbox_inches="tight")  
    return fig,ax
